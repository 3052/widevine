...
type Poster interface {
   ...
   Encode_Request(request []byte) ([]byte, error)
   Decode_Response(response []byte) ([]byte, error)
}

func (m Module) Post(post Poster) ([]byte, error) {
   body, err := func() ([]byte, error) {
      b, err := m.signed_request()
      if err != nil {
         return nil, err
      }
      return post.Encode_Request(b)
   }()
   if err != nil {
      return nil, err
   }
...
func _New_Module(private_key, client_ID, pssh []byte) (*Module, error) {
   pssh = pssh[32:]
   var mod Module
   // key_ID
   {
